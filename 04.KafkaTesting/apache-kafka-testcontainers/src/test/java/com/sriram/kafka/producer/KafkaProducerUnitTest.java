package com.sriram.kafka.producer;

import com.sriram.kafka.producer.EvenOddPartitioner;
...
class KafkaProducerUnitTest {
    ...
    void givenKeyValue_whenSend_thenVerifyHistory() throws ExecutionException, InterruptedException {
        ...
        Future<RecordMetadata> recordMetadataFuture = kafkaProducer.send("data", "{\"site\" : \"sriram\"}");
    ...
    void givenKeyValue_whenSend_thenSendOnlyAfterFlush() {
        ...
        //when
        kafkaProducer = new KafkaProducer(mockProducer);
        Future<RecordMetadata> record = kafkaProducer.send("data", "{\"site\" : \"sriram\"}");
    ...
    void givenKeyValue_whenSend_thenReturnException() {
        ...
        //when
        kafkaProducer = new KafkaProducer(mockProducer);
        Future<RecordMetadata> record = kafkaProducer.send("site", "{\"site\" : \"sriram\"}");
    ...
    void givenKeyValue_whenSendWithPartitioning_thenVerifyPartitionNumber() throws ExecutionException, InterruptedException {
        ...
        //when
        kafkaProducer = new KafkaProducer(mockProducer);
        Future<RecordMetadata> recordMetadataFuture = kafkaProducer.send("partition", "{\"site\" : \"sriram\"}");
